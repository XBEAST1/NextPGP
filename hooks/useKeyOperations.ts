...
export function useKeyOperations({
  ...
  const updateKeyPassword = useCallback(async (userId: any, newArmoredKey: any) => {
  ...
  const exportPublicKey = useCallback((user: any, { setPublicKeySnippet, setpublicKeyModal }: any) => {
  ...
  const backupSubkey = useCallback(
    async (subkey: any, subkeyIndex: any, armoredSubkey: any) => {
      try {
  ...
  const certifyUserKey = useCallback(
    async (certifierUser: any, targetUser: any, { setcertifyUserModal }: any) => {
      try {
        ...
        const existingKeyIds = theirPub.users.flatMap((user: any) =>
          user.otherCertifications.map((sig: any) => sig.issuerKeyID.toHex().toLowerCase())
        );
        ...
        const certifiedKey = await theirPub.signAllUsers([primaryKeyOnly as any], new Date());
  ...
  const ChangeKeyValidity = useCallback(
    async ({ isNoExpiryChecked, expiryDate, setvalidityModal, setSelectedUserId }: any) => {
      ...
      try {
        ...
        if (isNoExpiryChecked || !expiryDate) {
          ...
          keyExpirationTime = Math.floor(((expiry as any) - (now as any)) / 1000);
        }

        let privateKey: any = await openpgp.readPrivateKey({ armoredKey: selectedUserId.privateKey });
        ...
        fullPublicKey.users.forEach((user) => {
          if (user.userID) {
            userRevocationMap.set(user.userID.userID, {
              isRevoked: (user as any).isRevoked(),
        ...
        const allUserIDsForReformat = fullPublicKey.users
          ...
          .map((u: any) => (u.email && u.email !== "N/A" ? { name: u.name, email: u.email.trim() } : { name: u.name }));

        const originalSubkeys = privateKey.getSubkeys();
        const subkeyRevocationMap = new Map();
        originalSubkeys.forEach((sk: any) => {
        ...
        updatedPrivateKey.getSubkeys().forEach((sk: any) => {
          const fp = sk.getFingerprint();
          const orig = subkeyRevocationMap.get(fp);
          if (orig?.isRevoked) {
            orig.revocationSignatures.forEach((sig: any) => {
              if (!sk.revocationSignatures.some((e: any) => e.equals(sig))) sk.revocationSignatures.push(sig);
            });
          }
        });

        // Re-apply userID revocations
        updatedPrivateKey.users.forEach((user: any) => {
          if (!user.userID) return;
          const orig = userRevocationMap.get(user.userID.userID);
          if (orig?.isRevoked) {
            orig.revocationSignatures.forEach((sig: any) => {
              if (!user.revocationSignatures.some((e: any) => e.equals(sig))) user.revocationSignatures.push(sig);
            });
          }
        });
  ...
  const ChangeSubkeyValidity = useCallback(
    async (armoredSelectedSubkey: any, { isNoExpiryChecked, expiryDate, setvalidityModal, setSelectedUserId, setSelectedSubkey }: any) => {
      ...
      try {
        ...
        const targetSubkey = privateKey.subkeys.find((s: any) => s.keyPacket.getKeyID().toHex() === keyIDhex);
        if (!targetSubkey) throw new Error("Subkey not found on primary key");

        const fullPublicKey = await openpgp.readKey({ armoredKey: selectedUserId.publicKey });
        const existingUserIDs = fullPublicKey.getUserIDs().map(parseUserId).map((u: any) =>
        ...
        const updatedSubkey = helperKey.subkeys.find((s: any) => s.keyPacket.getKeyID().toHex() === keyIDhex);
        ...
        setUsers(refreshed);
        const updatedUser = refreshed.find((u: any) => u.id === selectedUserId.id);
  ...
  const removePasswordFromKey = useCallback(
    async ({ setremovePasswordModal }: any) => {
      try {
  ...
  const addUserID = useCallback(
    async (user: any, { name, email, setName, setEmail, setaddUserIDModal, setNameInvalid, setEmailInvalid }: any) => {
      ...
      try {
        ...
        const expirationSeconds = expirationTime ? Math.floor(((expirationTime as any) - (creationTime as any)) / 1000) : undefined;
        ...
        updatedPrivateKey.users.forEach((u) => {
          ...
          if (orig?.isRevoked) {
            orig.revocationSignatures.forEach((sig: any) => {
              if (!u.revocationSignatures.some((e: any) => e.equals(sig))) u.revocationSignatures.push(sig);
            });
          }
        });
  ...
  const setPrimaryUserID = useCallback(
    async (user: any, targetUserIDObj: any, { setModalUserIDs }: any) => {
      try {
        ...
        const updatedUser = refreshed.find((u: any) => u.id === user.id);
  ...
  const addSubkey = useCallback(
    async (user: any, { selectedAlgorithm, subkeyOption, isNoExpiryChecked, expiryDate, setaddSubkeyModal }: any) => {
      ...
      try {
        ...
        privateKey.getSubkeys().forEach((sk) => {
          ...
          if (orig?.isRevoked) {
            orig.revocationSignatures.forEach((sig: any) => {
              if (!sk.revocationSignatures.some((e: any) => JSON.stringify(e) === JSON.stringify(sig))) {
  ...
  const RevokeUsingCertificate = useCallback(
    async (user: any, revocationCertificate: any, { setKeyInput }: any) => {
      setKeyInput("");
      try {
        if (user.privateKey && user.privateKey.trim()) {
          ...
          const revokedKey = (await openpgp.revokeKey({ key: publicKey, format: "armored", revocationCertificate, date: new Date() })) as any;
  ...
  const revokeSubkey = useCallback(
    async (subkeyIndex: any, { setSelectedUserId: setSelUsr }: any) => {
      setRevocationReasonText("");
      try {
        ...
        if (primaryKey.isPrivate() && !primaryKey.isDecrypted()) {
          currentPassword = await triggerKeyPasswordModal(selectedUserId);
          primaryKey = (await openpgp.decryptKey({ privateKey: primaryKey as any, passphrase: currentPassword })) as any;
        }
        ...
        const refreshedKeys: any = await loadKeysFromIndexedDB();
        setUsers(refreshedKeys);
        const updated = refreshedKeys.find((u: any) => u.id === selectedUserId.id);
  ...
  const getRevocationReason = useCallback(async (user: any) => {
  ...
  const getUserIDsFromKeyForModal = useCallback(async (user: any) => {
  ...
  const manageSubkeys = useCallback(async (user: any) => {
    if (!user || !user.privateKey) return [];
    try {
