...
export function useCloudTableState({ initialColumns, allColumns, storageKey }) {
  ...
  const getInitialVisibleColumns = () => {
    ...
    try {
      ...
      const availableUids = allColumns.map((col) => col.uid);
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        otherPages = JSON.parse(saved).filter((col) => !availableUids.includes(col));
      } catch {}
    }
