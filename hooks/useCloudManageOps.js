...
export const processKey = async (key, vaultAuth, storedKeys, keyIndex) => {
  try {
...
export function useCloudManageOps({
    ...
    try {
      ...
      if (apiCacheRef.current[cacheKey]) {
        ...
        const processPromises = plainKeys.map(async (key, index) => {
          if (decryptedCacheRef.current[key.id]) return decryptedCacheRef.current[key.id];
          const decrypted = await processKey(key, vaultAuth, storedKeys, index).catch((err) => {
            console.error("Error processing key:", err);
            return null;
          });
          if (decrypted) decryptedCacheRef.current[key.id] = decrypted;
          return decrypted;
        });
      ...
      const encryptedCache = await Promise.all(
        paginatedKeys.map(async (key) => await encryptData(key, encryptionKey))
      );
  ...
  const importFromCloud = useCallback(async (selectedUser) => {
    try {
      ...
      if (!exists) {
        ...
        setUsers((prevUsers) => prevUsers.map((u) => u.id === selectedUser.id ? { ...u, status: "Imported" } : u));
      } else {
        const keyType = keyData.privateKey === null ? "Public Key" : "Keyring";
  ...
  const deleteKey = useCallback(async (user) => {
    try {
