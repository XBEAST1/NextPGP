...
export const processKey = async (key, validDecryptedBackedUpKeys = []) => {
...
export function useCloudBackupOps({
    ...
    try {
      ...
      request.onsuccess = async (e) => {
        ...
        if (neededKeys.length === rowsPerPage) {
          ...
          setUsers(decryptedKeys);
          setCache((prev) => ({ ...prev, [cacheKey]: neededKeys }));
        } else {
          const offset = (page - 1) * rowsPerPage;
          ...
          setCache((prev) => ({ ...prev, [cacheKey]: encryptedForCache }));
          setUsers(keys);
        }
  ...
  const backupKey = useCallback(async (user) => {
    try {
      ...
      if (hasPrivate) {
        const [hash, encrypted] = await Promise.all([
          workerPool({ type: "hashKey", responseType: "hashKeyResponse", text: privateKeyRaw }, addToast),
          workerPool({
            type: "encrypt",
      ...
      if (!hasPrivate && hasPublic) {
        const [hash, encrypted] = await Promise.all([
          workerPool({ type: "hashKey", responseType: "hashKeyResponse", text: user.publicKey }, addToast),
          workerPool({
            type: "encrypt",
            responseType: "encryptResponse",
            text: user.publicKey,
      ...
      if (response.ok) {
        if (responseData.message === "Key already backed up.") {
          ...
          setUsers((prevUsers) =>
            prevUsers.map((u) => (u.id === user.id ? { ...u, status: "Backed Up" } : u))
          );
        }
      } else {
        if (response.status === 429) {
