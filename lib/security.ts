...
import { executeRateLimit, type RateLimitOptions, type RateLimitResult } from '@/lib/rate-limiter';
...
export async function rateLimit(
  ...
  return executeRateLimit(options);
}
...
export function addSecurityHeaders(response: NextResponse): NextResponse {
  ...
  response.headers.set('Content-Security-Policy', "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval' https://va.vercel-scripts.com; style-src 'self' 'unsafe-inline'; img-src 'self' data: https: blob:; font-src 'self' data:; connect-src 'self' https://keyserver.ubuntu.com https://keys.openpgp.org https://authjs.dev https://va.vercel-scripts.com https://vitals.vercel-insights.com; frame-ancestors 'none';");
  return response;
}
