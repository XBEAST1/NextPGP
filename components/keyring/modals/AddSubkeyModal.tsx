...
interface AddSubkeyModalProps {
  ...
  subkeyOption?: string;
  setSubkeyOption?: (opt: string) => void;
  onAdd?: () => void;
}
    ...
    <Modal backdrop="blur" isOpen={isOpen} onClose={onClose}>
      <ModalContent className="p-8">
          ...
          onSelectionChange={(selectedItem) => {
            const selectedKey =
              typeof selectedItem === "object" && selectedItem !== null
                ? (selectedItem as any).key
                : selectedItem;
            if (selectedKey) setSelectedAlgorithm?.(selectedKey as string);
          }}
        >
          {(item) => (
        ...
        <DatePicker
          ...
          maxValue={(primaryKeyMaxDate as any) ?? undefined}
