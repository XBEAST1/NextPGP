...
export default function App() {
  ...
  const onSubmitPassword = useRef<((passphrase: string) => void) | null>(null);
  const keyPassphraseInputRef = useRef<HTMLInputElement>(null);
  ...
  useEffect(() => {
    ...
    const fetchKeysFromIndexedDB = async () => {
      try {
        const keysFromStorage = (await getStoredKeys()) as PGPKey[];

        const validKeys = await Promise.all(
          keysFromStorage.map(async (key: PGPKey) => {
            try {
              ...
              return { ...key, userIDs, canEncrypt } as PGPKey;
            } catch (error) {
  ...
  useEffect(() => {
    ...
    const newRecipients = recipients.filter((r) => {
      if (r === "") return true;
      if (typeof r === "object" && r !== null && "keyId" in r) {
        return validKeyIds.has(r.keyId);
      }
      return validKeyIds.has(r as string);
    });
  ...
  const handleSignerSelection = async (selectedItem: React.Key | null) => {
    const db = (await openDB()) as IDBDatabase;
    const transaction = db.transaction(
      [dbPgpKeys, selectedSigners],
      "readwrite"
