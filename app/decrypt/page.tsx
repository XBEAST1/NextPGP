...
export default function App() {
  ...
  const [pgpKeys, setPgpKeys] = useState<any>(null);
  const [password, setPassword] = useState("");
  const [isPasswordModalOpen, setIsPasswordModalOpen] = useState(false);
  const [files, setFiles] = useState<any>(null);
  const [currentPrivateKey, setCurrentPrivateKey] = useState<any>(null);
  ...
  const [currentPasswordFile, setCurrentPasswordFile] = useState<any>(null);

  // Refs for tracking processed and downloaded files
  const processedFilesRef = useRef<Set<any>>(new Set());
  const downloadedFilesRef = useRef<Set<any>>(new Set());
  const downloadLockRef = useRef<Set<any>>(new Set()); // Lock for preventing concurrent downloads of the same file
  const downloadQueueRef = useRef<any[]>([]); // Global download queue to ensure sequential downloads
  ...
  const appendDetail = (payload: any) => {
    setDetails((prev) => {
  ...
  const downloadQueue = async (filePayload: any = null) => {
  ...
  const handleDecrypt = async () => {
    ...
    try {
      if (inputMessage) {
        await new Promise<any>((resolve, reject) => {
          workerPool({
            ...
            onDecryptedMessage: (payload: any) => {
            ...
            onDetails: (payload: any) => appendDetail(payload),
            onToast: (toast: any) => addToast(toast),
            onModal: (isOpen: any) => {
              if (isOpen) {
                setIsPasswordModalOpen(true);
              }
      ...
      for (const file of uniqueFiles as any[]) {
        try {
          ...
          await new Promise<any>((resolve) => {
            workerPool({
              ...
              onDecryptedFile: async (filePayload: any) => {
                if (filePayload?.fileName && filePayload.decrypted) {
              ...
              onError: () => {
                resolve(null); // Continue to next file
              },
              onDetails: (payload: any) => appendDetail(payload),
              onToast: (toast: any) => {
              ...
              onCurrentPrivateKey: setCurrentPrivateKey,
            }).catch(() => resolve(null)); // Continue even if this file fails
          });
        } catch {}
      }
      ...
      if (filesNeedingPassword.size > 0) {
        ...
        if (!(isPasswordModalOpen && inputMessage && !currentPasswordFile)) {
          processNextPasswordFile(filesNeedingPassword as any);
        }
      }
    } catch {
    } finally {
      ...
      if (currentPasswordFile) {
        ...
        try {
          ...
          if (files && files.length) {
            ...
            const remainingFiles = (uniqueFiles as any[]).filter(
              (f) => !decryptedWithMsgPassword.has(f.name)
            );
            const successfullyDecryptedWithKeys = new Set();

            for (const file of remainingFiles as any[]) {
              try {
                const result = await new Promise<any>((resolve) => {
                  workerPool({
                    type: "fileDecrypt",
            ...
            // Update processed files - downloads will be handled by the download queue
            allSuccessfullyDecrypted.forEach((file: any) => {
              processedFilesRef.current.add(file.name);
            });
  ...
  const removeDuplicateDetails = (detailsStr: string) => {
