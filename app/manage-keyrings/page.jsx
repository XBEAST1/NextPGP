...
const INITIAL_VISIBLE_COLUMNS = ["name", "email", "age", "status", "actions"];
...
export default function App() {
  ...
  const loadKeysFromLocalStorage = () => {
    ...
    return keys.map((key) => ({
      ...
      email: key.email,
      age: "No Expiry",
      status: "active", // Have to implement the date logic
      avatar: "https://i.pravatar.cc/150?u=a042581f4e29026024d",
  ...
  useEffect(() => {
    const pgpKeys = loadKeysFromLocalStorage();
    setUsers(pgpKeys);

    const handleStorageChange = () => {
      const updatedKeys = loadKeysFromLocalStorage();
  ...
  const deleteKey = (userId) => {
    ...
    const refreshedKeys = loadKeysFromLocalStorage();
