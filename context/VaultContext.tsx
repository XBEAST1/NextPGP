...
interface VaultContextType {
  encryptedVaultPassword: { iv: number[]; data: number[] } | null;
  encryptedVaultKeyData: { iv: number[]; data: number[] } | null;
  isVaultUnlocked: boolean;
  unlockVault: (password: string, verificationCipher?: string | null) => Promise<void>;
  lockVault: () => Promise<void>;
  getVaultPassword: () => Promise<string | null>;
  getVaultKeyMaterial: () => Promise<{ password: string | null; keyMaterial: Uint8Array | null; vaultSalt: Uint8Array | null }>;
}

declare global {
  interface Window {
    vaultLockInProgress?: boolean;
  }
}
...
export const VaultProvider = ({ children }: { children: ReactNode }) => {
  const [encryptedVaultPassword, setEncryptedVaultPassword] = useState<{ iv: number[]; data: number[] } | null>(null);
  const [encryptedVaultKeyData, setEncryptedVaultKeyData] = useState<{ iv: number[]; data: number[] } | null>(null);
  const [isVaultUnlocked, setIsVaultUnlocked] = useState(false);

  // Encrypt the vault password & master key material and store them in memory
  const unlockVault = async (password: string, verificationCipher: string | null = null) => {
    try {
      const masterKey = await getEncryptionKey() as CryptoKey;
