...
interface PasswordProtectionContextType {
  ...
  removePassword: (password: string) => Promise<{ success: boolean }>;
  verifyPassword: (password: string) => Promise<{ success: boolean }>;
}

const PasswordProtectionContext = createContext<PasswordProtectionContextType | null>(null);
...
const PasswordSetupModal = ({ isOpen, onClose, onPasswordSet }: { isOpen: boolean, onClose: () => void, onPasswordSet: (password: string) => Promise<void> }) => {
...
const PasswordUnlockModal = ({
  ...
  isOpen: boolean;
  onClose: () => void;
  onPasswordVerified: (password: string) => Promise<{ success: boolean }>;
...
const PasswordRemoveModal = ({ isOpen, onClose, onPasswordVerified }: { isOpen: boolean, onClose: () => void, onPasswordVerified: (password: string) => Promise<{ success: boolean }> }) => {
  const [password, setPassword] = useState("");
  const [showPassword, setShowPassword] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState("");
...
const DeleteDataModal = ({ isOpen, onClose, onDeleteData }: { isOpen: boolean, onClose: () => void, onDeleteData: () => Promise<void> }) => {
...
export const PasswordProtectionProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    const checkPasswordProtection = async () => {
      try {
        const hasPassword = await checkIfPasswordProtected() as boolean;
